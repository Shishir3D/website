...
//   node render.mjs --frames --workers=4                                         full-res JPEG frames → out/frames (resumable)
//   node render.mjs --encode [--out=out/video.mp4]                               frames + song → MP4
...
const SONG = new Function(readFileSync('song.js', 'utf8') + '; return SONG;')();
...
const DUR = SONG.dur, fps = +(args.fps || 24);
const audioIn = (...pre) => SONG.audio ? [...pre, '-i', SONG.audio] : [];
const audioOut = () => SONG.audio ? ['-map', '1:a', '-c:a', 'aac', '-b:a', '192k', '-shortest'] : [];
...
if (args.encode) {
  ...
  await run('ffmpeg', ['-y', '-loglevel', 'error', '-stats', '-framerate', String(fps), '-i', `${FRAMES_DIR}/f%05d.jpg`, ...audioIn(),
    '-map', '0:v', ...audioOut(), '-c:v', 'libx264', '-preset', 'slow', '-crf', '17', '-pix_fmt', 'yuv420p', '-movflags', '+faststart', out]);
...
const browser = await puppeteer.launch({
  ...
  args: ['--allow-file-access-from-files', '--ignore-gpu-blocklist', ...(process.platform === 'win32' ? ['--use-angle=d3d11'] : ['--use-angle=gl']), '--enable-gpu-rasterization', '--window-size=1920,1080', '--disable-renderer-backgrounding', '--disable-background-timer-throttling']
});
...
if (args.sheet) {
  ...
  const ff = spawn('ffmpeg', ['-y', '-loglevel', 'error', '-f', 'image2pipe', '-framerate', String(fps), '-c:v', 'mjpeg', '-i', '-',
    ...audioIn('-ss', String(a), '-t', String(b - a)),
    '-map', '0:v', ...audioOut(), '-c:v', 'libx264', '-preset', 'medium', '-crf', '19', '-pix_fmt', 'yuv420p', out],
